...
app.get("/liked-artists", async (req, res) => {
  ...
  const time_1 = await mongo.databaseQuery(
    "token_created",
    req.cookies.user_id
  );
  ...
  if (Date.now() - time_1 <= 3500000) {
    try {
      ...
      const access = await mongo.databaseQuery(
        "access_token",
        req.cookies.user_id
      );
    ...
    try {
      ...
      const refresh = await mongo.databaseQuery(
        "refresh_token",
        req.cookies.user_id
      );
...
app.get("/concerts", (req, res) => {
  console.log("getting conert data");
  try {
    ...
    getConcerts(artist, state).then((concerts) => {
